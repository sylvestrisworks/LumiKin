/**
 * Rescore games with bundledOnlineNote using the base-game-only prompt.
 * Targets GTA V and RDR2 (Minecraft's current score already reflects the base game).
 *
 * Usage:
 *   node --env-file=.env node_modules/tsx/dist/cli.cjs scripts/rescore-base-game.ts
 *   node --env-file=.env node_modules/tsx/dist/cli.cjs scripts/rescore-base-game.ts --all
 */
...
// Default targets: GTA V and RDR2 (Minecraft already looks correct at cura=71)
const DEFAULT_SLUGS = ['grand-theft-auto-v', 'red-dead-redemption-2']

// ─── Review tool (mirrors route.ts) ──────────────────────────────────────────
...
const REVIEW_TOOL = {
  ...
  input_schema: {
    ...
    properties: {
      b1_cognitive:    scoreGroup(B1, 5, 'B1 cognitive scores, each 0–5'),
      b2_social:       scoreGroup(B2, 5, 'B2 social-emotional scores, each 0–5'),
      b3_motor:        scoreGroup(B3, 5, 'B3 motor scores, each 0–5'),
      r1_dopamine:     scoreGroup(R1, 3, 'R1 dopamine manipulation scores, each 0–3'),
      r2_monetization: scoreGroup(R2, 3, 'R2 monetization pressure scores, each 0–3'),
      r3_social:       scoreGroup(R3, 3, 'R3 social/emotional risk scores, each 0–3'),
      r4_content:      scoreGroup(R4, 3, 'R4 content risk scores, each 0–3'),
      r4_modifiers: {
      ...
      representation: scoreGroup(REP, 3, 'REP representation scores, each 0–3'),
      propaganda: {
...
function buildPrompt(g: typeof games.$inferSelect): string {
  ...
  return [
    ...
    '',
    '## CALIBRATION',
    'GTA V single-player campaign (expected baseline): B1~30, B2~12, B3~12 | R1~8, R2~0, R3~6 → curascore ~65, 90–120 min/day',
    'RDR2 single-player campaign (expected baseline):  B1~28, B2~18, B3~10 | R1~6, R2~0, R3~4 → curascore ~70, 120 min/day',
    '',
    '## GAME TO REVIEW',
    `Title: ${g.title}`,
    ...
    'This title ships with a separate online/live-service mode covered by a dedicated caution notice shown to parents.',
    'Score ONLY the single-player or base offline campaign. Set all R1/R2/R3 flags that belong to the online mode to 0.',
    'This includes: live-service loops, real-money currency (Shark Cards / Gold Bars), stranger chat, FOMO events, social pressure.',
    'The narratives must describe the base game experience. Do not mention the online mode in the score fields.',
    '',
...
async function saveReview(game: typeof games.$inferSelect, r: ReviewInput): Promise<number> {
  ...
  // Ensure needsRescore is cleared
...
async function main() {
  console.log('\n╔══════════════════════════════════════════════════╗')
  console.log('║  Rescore bundled-online games — base game only  ║')
  console.log('╚══════════════════════════════════════════════════╝\n')

  // Fetch targets
  let targets: (typeof games.$inferSelect)[]
  if (ALL_FLAG) {
    ...
    const rows = await db.select().from(games)
      .where(isNotNull(games.bundledOnlineNote))
    targets = rows.filter(g => DEFAULT_SLUGS.includes(g.slug))
  }
  ...
  console.log(`Targets: ${targets.map(g => g.title).join(', ')}\n`)
