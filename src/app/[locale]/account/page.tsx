  ...
  return (
    <div className="min-h-screen bg-paper text-ink">
      <main className="max-w-lg mx-auto px-4 py-10 space-y-6">
        ...
        <h1 className="font-serif text-display-sm text-ink border-b border-ink pb-4">{t('title')}</h1>

        {/* ── Profile ─────────────────────────────────────────────────────── */}
        <div className="border border-rule px-5 py-4 flex items-center gap-4">
          {image ? (
            // eslint-disable-next-line @next/next/no-img-element
            <img src={image} alt="" referrerPolicy="no-referrer" className="w-12 h-12 rounded-full shrink-0 ring-1 ring-rule" />
          ) : (
            <div className="w-12 h-12 rounded-full bg-ink text-paper text-sm font-bold flex items-center justify-center shrink-0">
              {(name ?? email ?? '?').split(' ').slice(0, 2).map(w => w[0]?.toUpperCase()).join('')}
            </div>
          )}
          <div className="min-w-0">
            {name && <p className="font-serif text-ink truncate">{name}</p>}
        ...
        <section>
          ...
          <div className="border border-rule px-5 py-4 space-y-3">
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs text-muted truncate max-w-[160px]">{email}</span>
            </div>
        ...
        <section>
          ...
          <div className="border border-rule px-5 py-4 space-y-2">
            <Link href={`/${locale}/privacy`} className="block text-sm text-accent hover:underline">
              {t('privacyPolicy')}
            </Link>
            <Link href={`/${locale}/terms`} className="block text-sm text-accent hover:underline">
              {t('termsOfService')}
            </Link>
          </div>
        </section>
