...
export default async function FortniteMapPage({ params }: Props) {
  ...
  const reviewBodyRaw = score?.summary
    ?? (score?.curascore != null
      ? `${exp.title} received a LumiScore of ${score.curascore}/100 on Fortnite Creative.${score.timeRecommendationLabel ? ` Recommended play time: ${score.timeRecommendationLabel}.` : ''}`
      : null)
