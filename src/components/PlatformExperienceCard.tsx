    ...
    >
      ...
      <div className="relative h-28 bg-rule/30 overflow-hidden shrink-0">
        ...
        {!isPending && exp.recommendedMinAge != null && (
          <div className="absolute bottom-1.5 left-1.5 bg-paper text-ink text-kicker uppercase font-semibold px-1.5 py-0.5 leading-none" style={{ fontVariantCaps: 'all-small-caps' }}>
      ...
      <div className="px-3 py-2.5 flex flex-col gap-1 flex-1">
        ...
        {exp.creatorName && (
          <span className="text-kicker uppercase text-muted self-start truncate max-w-full" style={{ fontVariantCaps: 'all-small-caps' }}>
            {exp.creatorName}
          </span>
        )}
        ...
        {!isPending && (hasHighStrangerRisk || hasHighMonetization) && (
          <div className="flex flex-wrap gap-x-2 gap-y-0.5">
            ...
            {hasHighMonetization && (
              <span className="text-xs text-warm">{t('spendPressure')}</span>
            )}
          </div>
        )}
      </div>
